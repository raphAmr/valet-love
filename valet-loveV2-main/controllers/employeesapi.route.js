...
async function employeeDeleteAction(request, response) {
    try {
        ...
        if (numRows === 0) {
            return response.status(404).json({ error: "employee non trouvé ou déjà supprimé." });
        }

        response.status(200).json({ message: `employee avec l'ID ${employeeId} supprimé avec succès.`, rowsDeleted: numRows });
    } catch (err) {
        console.error("Erreur lors de la suppression du employee : ", err);
        response.status(500).json({ error: 'Erreur lors de la suppression du employee.' });
    }
}
...
//version elo pour update
// Route pour mettre à jour un employé (déjà existante)
async function employeeUpdateAction(req, res) {
    try {
        ...
        if (numRows === 0) {
            return res.status(404).json({ error: "employee non trouvé ou aucune donnée mise à jour." });
        }

        res.status(200).json({ message: `employee avec l'ID ${employeeId} mis à jour avec succès.`, rowsUpdated: numRows });
    } catch (err) {
        console.error("Erreur lors de la mise à jour du employee : ", err);
        res.status(500).json({ error: 'Erreur lors de la mise à jour du employee.' });
    }
}
