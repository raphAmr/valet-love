...
export default {
  ...
  mounted() {
    ...
    this.loadBarWorkers(barId); // Utilise l'identifiant dynamique
    this.bar = barsData.find(bar => bar.id_bar === barId);
    //version elo (pour add):
    
    },


    //verison elo (pour add):
  ...
  try {
    console.log("Chargement des employés pour le bar :", id_bar);
    let responseEmployees = await fetch('http://localhost:3000/employees/list');

    if (!responseEmployees.ok) {
      // En cas de code différent de 200, on récupère le texte (par exemple : "Aucun employee trouvé.")
      const errorMessage = await responseEmployees.text();
...
async addEmployee() {
  try {
    // On récupère l'ID du bar actuel
    const barId = parseInt(this.$route.params.barId, 10);

    // Données par défaut pour le nouvel employé
    ...
    if (!response.ok) {
      throw new Error(`Erreur lors de la création de l'employé: ${response.statusText}`);
    }

    const createdEmployee = await response.json();
    console.log("Nouvel employé créé :", createdEmployee);

    // Recharger la liste des employés du bar courant
    await this.loadBarWorkers(barId);
    
    // La liste dans `this.barWorkers` sera mise à jour, 
    // et l'affichage se rafraîchira automatiquement grâce au data-binding Vue
  } catch (error) {
...
//version elo pour supp et update
// Fonction pour activer le mode édition d'un worker
editWorker(index) {
  ...
  try {
    ...
    // Ici, on vérifie si response.ok est false :
    if (!response.ok) {
    ...
    // Si on arrive ici, c'est que la requête s'est bien passée et la réponse est du JSON valide
