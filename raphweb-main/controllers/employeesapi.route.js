...
async function employeeCreateAction(req, res) {
    try {
        ...
        const {
            ...
            post_employee = "post",
            salary_employee = 0,
            id_bar = 1, // ID du bar par défaut
        } = req.body;

        // Vérification des données
        if (!name_employee || age_employee < 0 || !gender_employee || !post_employee || salary_employee < 0) {
            return res.status(400).send("Les données fournies ne sont pas valides.");
        }

        console.log("Tentative de création d'un employé avec les données :", req.body);

        // Appel de la fonction du repository pour créer un employé
        const newEmployeeId = await employeeRepo.createEmployee({
            name_employee,
            age_employee,
            gender_employee,
        ...
        // Vérifier si l'employé a été créé
        if (!newEmployeeId) {
            return res.status(500).send("Erreur lors de la création de l'employé.");
        }

        res.status(201).send(`Employé créé avec succès avec l'ID : ${newEmployeeId}`);
    } catch (err) {
